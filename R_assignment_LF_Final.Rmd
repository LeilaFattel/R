...
### Inspect files by using head() and tail() (I will not demonstrate here as it will take lots of space in the RMarkdown file). We can also determine the number of columns and rows using ncol() and nrow(), respectively. Use names() to list the headers in both files and class() to determine the class of the files. I added dim() in this chunk and and another way to look at a few lines from both files as one of my reviewers suggested.
```{r}
ncol(fang_genotypes)
ncol(snp_position)
nrow(fang_genotypes)
...
dim(fang_genotypes)
dim(snp_position)
fang_genotypes[1:5,1:10]
snp_position[1:3, 1:5]
```
